...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('prototypes', torch.nn.functional.normalize(torch.zeros(3, 128), p=2, dim=1))
    ...
    def update(self, features, labels):
        ...
        for cls_id in range(3):
            indices = torch.nonzero(labels == cls_id).reshape(-1)
            if indices.numel() == 0:
                continue
            cls_features = features[indices]
            cls_mean = torch.nn.functional.normalize(cls_features.mean(dim=0), p=2, dim=0)
            self.prototypes.data[cls_id] = self.prototypes[cls_id] * self.m + (1 - self.m) * cls_mean
        self.prototypes.data.copy_(torch.nn.functional.normalize(self.prototypes, p=2, dim=1))
        return
